...
// drops richlist data for given coin
  ...
  create_cmc: function(coin, cb) {
    var newCmc = new Cmc({
      ...
      percent_change_7d: 0,
      last_updated: "2020-01-27T16:59:56.350Z"
    });
  ...
  update_coinmarketcap_db: function(coin, cb) {
    coinmarketcap.get_data(coin, function(err, obj) {
      ...
      Cmc.update({ symbol: coin }, {
        rank: obj.rank,
        price_usd: obj.quotes.USD.price,
        price_btc: obj.quotes.BTC.price,
        volume_24h_usd: obj.quotes.USD.volume_24h,
        market_cap_usd: obj.quotes.USD.market_cap,
        available_supply: obj.circulating_supply,
        total_supply: obj.total_supply,
        percent_change_1h: obj.quotes.USD.percent_change_1h,
        percent_change_24h: obj.quotes.USD.percent_change_24h,
        percent_change_7d: obj.quotes.USD.percent_change_7d,
        last_updated: obj.last_updated
      }, function() {
        Stats.updateOne({coin:settings.coin}, {
          last_price: obj.quotes.BTC.price,
        }, function(){
          return cb(null);
        });
      });
  ...
  update_coingecko_db: function(coin, cb) {
    coingecko.get_data_usd(coin, function(err, obj_usd) {
      if (err !== null) {
        return cb(err);
      }
      coingecko.get_data_btc(coin, function(err, obj_btc) {
	    if (err !== null) {
          return cb(err);
        }
		var update = {
          rank: obj_usd.market_cap_rank.toString(),
          current_price: obj_usd.current_price.toString(),
          price_btc: obj_btc.current_price.toString(),
          total_volume: obj_usd.total_volume.toString(),
          market_cap: obj_usd.market_cap.toString(),
          circulating_supply: obj_usd.circulating_supply.toString(),
          total_supply: obj_usd.total_supply.toString(),
          price_change_percentage_1h_in_currency: obj_usd.price_change_percentage_1h_in_currency.toString(),
          price_change_percentage_24h_in_currency: obj_usd.price_change_percentage_24h_in_currency.toString(),
          price_change_percentage_7d_in_currency: obj_usd.price_change_percentage_7d_in_currency.toString(),
          last_updated: obj_usd.last_updated
        };
		Cmc.findOneAndUpdate({symbol: coin}, update, { upsert: true, useFindAndModify:false }, function(err) {
		  if(err !== null) {
		    return cb(err);
		  }
          Stats.findOneAndUpdate({coin:settings.coin}, {
            last_price: obj_btc.current_price,
          }, { upsert: true, useFindAndModify:false }, function(err){
			if(err !== null) {
			  return cb(err);
			}
            return cb(null);
